...
local_directory <- getwd();				# this is the directory from which you are running this R-program
common_source_folder <- paste(getwd(),"/R/",sep="");	# directory to folder where you keep common source
data_for_R_folder <- paste(getwd(),"/data/",sep="");	# directory to folder where you keep R-Data files
...
cincta <- accersi_PaleoDB_data_from_chosen_nexus_file(onset="Cambrian",end="Cambrian",rock_unit_databases=rock_unit_data,chronostratigraphic_databases=chronostratigraphic_databases,paleodb_fixes=paleodb_fixes,control_taxon="Echinodermata",zone_taxa=c("Trilobita","Archaeocyatha"),taxon_level="species",basic_environments=c("marine","unknown"),time_scale_stratigraphic_scale="Stage Slice",analysis_name="Cincta",bogarted=F);
