...
# modified 2020-03-02
# modified 2020-05-05
accersi_occurrence_data <- function(taxa,onset="Proterozoic",end="Holocene",basic_environments="terr,marine,unknown",species_only=T,clean_entered_taxa=T,directory="",save_files=T,output_type=".csv") {
# Arguments:
...
#		desired_finds <- rbind(subset(all_finds,all_finds$identified_rank=="species"),subset(all_finds,all_finds$identified_rank=="subspecies"))
	...
	if (nrow(desired_finds)==0)	{
		...
		flags1 <- sapply(taxon_name,revelare_uncertain_species_assignments);
...
accersi_occurrences_for_one_taxon <- function(taxon,species_only=T)	{
...
# modified 2020-02-17
# modified 2020-05-18
accersi_collection_data <- function(taxa,onset="Archean",end="Cenozoic",basic_environments="terr,marine,unknown",paleogeography="gplates",standardize_members=F,directory="",save_files=T,species_only=F,output_type=".csv") {
taxa <- paste(taxa, collapse = ",");
taxa <- gsub(" ","%20",taxa);
if (!is.na(match("terrestrial",basic_environments)))
	basic_environments[match("terrestrial",basic_environments)] <- "terr";
basic_environments <- paste(basic_environments,collapse=",");
http <- paste("http://paleobiodb.org/data1.2/colls/list.csv?base_name=",taxa,"&interval=",onset,",",end,"&envtype=",basic_environments,"&pgm=",paleogeography,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
...
gorf <- 0;
while (gorf < length(geographic_orphans))	{
	gorf <- gorf + 1;
	...
	coll_no <- geographic_orphans[gorf];
	http <- paste("http://paleobiodb.org/data1.2/colls/single.csv?id=",coll_no,"&pgm=",pgm,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,refattr,ent,entname,crmod",sep="");
	site_reboot <- read.csv(http, header = TRUE, stringsAsFactors=hell_no);
	if (!is.na(site_reboot$paleolat))	{
		cn <- match(coll_no,collections$collection_no);
		collections$paleolat[cn] <- as.numeric(site_reboot$paleolat);
		collections$paleolng[cn] <- as.numeric(site_reboot$paleolng);
		collections$geoplate[cn] <- as.numeric(site_reboot$geoplate);
		}
...
#	zz <- c(zz,mundus_zone(zone[z]));
#	}
zone <- as.character(collections$zone[collections_w_zones]);
collections$zone[collections_w_zones] <- sapply(zone,mundus_zone);
...
#http <- paste("http://paleobiodb.org/data1.2/colls/list.csv?base_name=",taxa,"&interval=",onset,",",end,"&envtype=",basic_environments,"&pgm=",paleogeography,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
http <- paste("http://paleobiodb.org/data1.2/colls/list.csv?coll_id=",coll_id,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
fetch <- RCurl::getURL(http);
...
#httpC <- paste("http://paleobiodb.org/data1.2/colls/list.csv?id=",collection_no,"&pgm=",paleogeography,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
#http <- paste("http://paleobiodb.org/data1.2/colls/list.csv?base_name=",taxa,"&interval=",onset,",",end,"&envtype=",basic_environments,"&pgm=",paleogeography,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
httpC <- paste("http://paleobiodb.org/data1.2/colls/list.csv?id=",collection_no,"&pgm=",paleogeography,"&show=loc,paleoloc,strat,stratext,timebins,timecompare,lith,lithext,env,geo,methods,resgroup,ref,secref,refattr,ent,entname,crmod",sep="");
...
http <- paste("http://paleobiodb.org/data1.2/strata/list.csv?name=",rock_unit_url,"&show=gplates,splates",sep="");
...
accersi_taxonomic_data_for_one_taxon <- function(taxon,inc_children=F,oldest="",youngest="")	{
# taxon: taxon name;
...
if (inc_children)	{
	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?base_name=",taxon,"&variant=all&rel=all_children",interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	} else	{
#	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?match_name=",taxon,"&show=attr,parent,class,classext,refattr,crmod",sep="");
	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?match_name=",taxon,                   interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	}
...
accersi_taxonomic_data_for_one_taxon_no <- function(taxon_no,inc_children=F,oldest="",youngest="")	{
# taxon: taxon name;
# inc_subtaxa: if T, then taxonomic data for all constituent taxa are included; if F, then only for this taxon.
if (oldest!="" && youngest=="")	{
	youngest <- "Cenozoic";
...
if (inc_children)	{
#	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?id=txn:",taxon_no,"&variant=all&rel=all_children",interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref",sep="");
	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?id=txn:",taxon_no,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	} else	{
#	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?match_name=",taxon,"&show=attr,parent,class,classext,refattr,crmod",sep="");
	httpT <- paste("http://paleobiodb.org/data1.2/taxa/single.csv?id=txn:",taxon_no,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
#	httpT <- paste("http://paleobiodb.org/data1.2/taxa/single.txt?id=txn:",taxon_no,interval,"&show=attr,common",sep="");
#	httpT <- paste("http://paleobiodb.org/data1.2/taxa/list.csv?id=txn:=",taxon_no,interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	}
#             http://paleobiodb.org/data1.2/taxa/opinions.txt?base_name=Cypraeidae&rank=species,subspecies&op_type=all
#fetch <- RCurl::getURL(httpT);
#taxon_information <- utils::read.csv(text = fetch, header = TRUE, stringsAsFactors=FALSE,encoding="UTF-8");
...
species <- sapply(taxon,mundify_taxon_names);
...
taxon_list <- unique(sapply(taxon_list,mundify_taxon_names));
...
#accio <- RCurl::getURL(httpTO);
#http://www.paleobiodb.org/data1.2/opinions/list.json?author=Osborn
taxon_opinions <- read.csv(httpTO,header=T,stringsAsFactors=hell_no,encoding="UTF-8");
return(taxon_opinions);
}
...
expello_indeterminate_species <- function(paleodb_finds)	{
...
http <- paste("http://www.paleobiodb.org/data1.2/occs/refs.csv?base_name=",taxon,"&interval=",onset,",",end,"&select=",paste(ref_types,collapse=","),"&private&show=crmod",sep="");
...
#http <- paste("http://www.paleobiodb.org/data1.2/refs/list.txt?id=",reference_no,"&show=both",sep="");
#reference_no <- paste(reference_nos,collapse=",");
reference_no <- reference_nos;
pbdb_references <- data.frame(t(sapply(reference_no,accersi_reference_by_number)),stringsAsFactors = hell_no);
for (cc in 1:ncol(pbdb_references))	pbdb_references[,cc] <- as.vector(unlist(pbdb_references[,cc]));
#http <- paste("http://www.paleobiodb.org/data1.2/refs/single.txt?id=",reference_no,"&show=both",sep="");
...
http <- paste("http://www.paleobiodb.org/data1.2/refs/list.csv?ref_author=",author,"&show=both",sep="");
pbdb_references <- read.csv(http, header = TRUE, stringsAsFactors=FALSE,fileEncoding = "UTF-8");
...
#http <- paste("http://www.paleobiodb.org/data1.2/refs/list.txt?id=",reference_no,"&show=both",sep="");
#http <- paste("http://www.paleobiodb.org/data1.2/refs/single.txt?id=",reference_no,"&show=both",sep="");
#http <- paste("http://paleobiodb.org/data1.2/taxa/refs.csv?datainfo&rowcount&base_name=",taxon,"&select=auth,class,ops,occs,specs,colls",sep="")
...
if (reference$otherauthors!="" && tolower(style)=="full")	{
	...
	white_walkers <- white_walkers[!white_walkers %in% c("Jr"," Jr")];
...
#divido_lumped_rock_units(named_rock_unit)
...
if (sum(zone_molecularized %in% c("NP","CC"))>0)	{
	nps <- (1:length(zone_molecularized))[zone_molecularized %in% "NP"];
	for (nn in length(nps):1)	{
		npn <- nps[nn]+1;
		if (as.numeric(zone_molecularized[npn])>0 && as.numeric(zone_molecularized[npn])<30)	{
...
redone_collections_db <- redone_collections <- match_paleodb_collections_to_external_stratigraphic_database(collections,wagner_rocks=rock_database);
...
zone <- zone_data$zone;
new_zone_combos <- base::t(sapply(zone,accersi_genus_subgenus_combinations_for_zones));
#new_zone_combos <- c();
...
zone_genera_subgenera <- base::t(sapply(genus_name,divido_subgenus_names_from_genus_names));
abbreviated_zone <- c();
...
construct_stratigraphic_data_base_from_paleodb_old <- function(taxa,onset="Cambrian",end="Holocene",save_files=TRUE,directory="",output_type=".csv")	{
# taxa: taxon or list of taxa for which you want collection data
# onset: oldest rocks
# end: youngest rocks
# save_files: whether to save files
...
accersi_rock_order_in_section <- function(rock_no_sr,section)	{
...
#xxx <- sapply(pp,accersi_exact_prob_event_at_x_given_n_prior_and_m_later_events,p_event,prior=events_prior_start,later=events_after_start);
#yyy <- sapply(pp,accersi_exact_prob_event_at_x_given_n_prior_and_m_later_events,p_event,prior=events_prior_end,later=events_after_end);
...
#	lnp_end[te] <- lnp_now+log(dpois(events_prior_end,expected_prior_events));
	...
	for (te in ts:ttl_time_slices)	{
		## rescale time slices by start & get probs of last appearances using accersi_exact_prob_event_at_x_given_n_prior_and_m_later_events
...
output <- list(data.frame(poss_age=as.numeric(rock_order$ma_lb[this_rock_lo]-((time_slices)*(rock_order$ma_lb[this_rock_lo]-rock_order$ma_ub[this_rock_hi]))),
					 prob_rock_present=as.numeric(p_present),prob_start=as.numeric(p_start),
