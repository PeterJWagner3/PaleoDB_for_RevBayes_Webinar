...
accersi_rock_order_in_section <- function(rock_no,section,topdown=T)	{
...
cooccurrence_matrix_old <- function(finds) { 
S <- ncol(finds)
coccr_mat <- array(1,dim=c(S,S)) #sets all values in array to 1
sites <- nrow(finds)
for (i in 1:(S-1))  {
	for (j in (i+1):S)	{
		ii <- (1:sites)[finds[,i]==1] #sites with species i
		jj <- (1:sites)[finds[,j]==1] #sites with species j
		if (sum(ii %in% jj) == 0)  #true for all sites i and j share
			coccr_mat[i,j] = coccr_mat[j,i] = 0  #if i ≠ (!= in programming) j reset value to 0
		}
	}
return(coccr_mat);
}
