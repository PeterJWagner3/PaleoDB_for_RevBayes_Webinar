...
bogarted <- F;												# if true, then you wll be prompted for a file of additional occurrences; default=F;
...
# Now, let's set up the stratigraphic datafiles that RevBayes wants
...
taxon_names <- sapply(taxon_name,newickify_taxon_name);
# We'll generate 5 files
# 1st: just the upper & lower bounds of the stratigraphic range using millions of years ago
fossil_intervals_for_RevBayes_raw <- data.frame(taxon=as.character(taxon_names),min=as.numeric(strat_range_summaries$latest_poss_la),max=as.numeric(strat_range_summaries$earliest_poss_fa),stringsAsFactors = hell_no);
fossil_intervals_for_RevBayes_raw$min <- round(fossil_intervals_for_RevBayes_raw$min,output_precision);
fossil_intervals_for_RevBayes_raw$max <- round(fossil_intervals_for_RevBayes_raw$max,output_precision);
...
fossil_intervals_for_RevBayes <- data.frame(taxon=as.character(taxon_names),min=as.numeric(strat_range_summaries$latest_poss_la-faux_present),max=as.numeric(strat_range_summaries$earliest_poss_fa-faux_present),stringsAsFactors = hell_no);
...
fossil_intervals_for_RevBayes_FA_only <- data.frame(taxon=as.character(taxon_names),min=as.numeric(strat_range_summaries$earliest_poss_fa-faux_present2),max=as.numeric(strat_range_summaries$earliest_poss_fa-faux_present2),stringsAsFactors = hell_no);
# 4th: upper and lower bounds for both first and last appearances, again set to the latest possible LA is 0. 
fossil_intervals_for_RevBayes_fuzzy <- data.frame(taxon=as.character(taxon_names),min_mn=as.numeric(strat_range_summaries$latest_poss_la-faux_present),min_mx=as.numeric(strat_range_summaries$earliest_poss_la-faux_present),max_mn=as.numeric(strat_range_summaries$latest_poss_fa-faux_present),max_mx=as.numeric(strat_range_summaries$earliest_poss_fa-faux_present),stringsAsFactors = hell_no);
...
fossil_intervals_for_RevBayes_occurrences$taxon <- sapply(taxon_name,newickify_taxon_name);
...
write.table(fossil_intervals_for_RevBayes_fuzzy,file=paste(local_directory,fossil_interval_file_raw,sep=""),row.names = F,sep="\t",quote = F);
...
psi <- mean(((rowMeans(exp_finds_sites_quantiles[study_bins,])+rowMeans(exp_finds_rocks_quantiles[study_bins,]))/2)/finest_chronostrat$span[match(study_bins,finest_chronostrat$interval)]);
...
rho <- (mean(as.numeric(pfind_rocks_quantiles[last_bin,]))+mean(as.numeric(pfind_sites_quantiles[match(last_bin,rownames(pfind_rocks_quantiles)),])))/2;
# average sampling
psi <- mean(((rowMeans(exp_finds_sites_quantiles[study_bins,])+rowMeans(exp_finds_rocks_quantiles[study_bins,]))/2)/finest_chronostrat$span[match(study_bins,finest_chronostrat$interval)]);
...
### Output MCMC & Stepping Stone Scripts ###
